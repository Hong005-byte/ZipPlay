...
- **22 套皮肤**，启动时或运行中随时可切换：
  ...
  - 🎨 **自定义主题**：自定义主题页里能自己写主题——不是代码，是纯数据（JSON）：颜色、渐变背景、像素图标（行数/宽度各自 4~64，不用是正方形；展示位就 20~52px 上下，图省事的话 8x8 就够）、从 8 种内置动画（呼吸发光/渐隐渐现/飘过/飘落/上下浮动/摇摆/旋转/不规则闪烁）里选一种（也可以用 `+` 连起来同时挂好几招，比如 `pulse+sway` 就是呼吸发光的同时还在摇摆），还能勾一个 `musicReactive` 开关让这个动画跟着音乐变速，再配一档 `sensitivity`（低/中/高）调它对响度/鼓点"反应多强"。格式错了会告诉你具体哪个字段错，最多能存 10 个，页面里带一份完整示例照着改就行。
    ...
    - **🖌️ 图标画板**：不想再自己数字符网格的话，点格子画图标——调色板（预置 8 色 + 自己加）、拖拽连续涂色/擦除、左右镜像绘制（画一边自动镜像另一边，对称图标能省将近一半点击量）、实际大小预览。画完能直接替换当前 JSON 里的 `icon` 字段（其它字段不动），也能只复制 JSON 片段贴给 `layers` 用。打开时如果输入框里已经有一个图标（手写的或者之前画过的），会先摊开续画，不是每次都从空白开始。
      - **帧列表（逐帧动画）**：底部一条"帧"缩略图，➕ 新增帧 / 📋 复制当前帧（走路循环这种大部分帧只是小范围挪动，从上一帧改比每次从空白画快得多）/ 🗑 删除当前帧，调色板是所有帧共用的一份，改尺寸也是对全部帧一起生效（保证帧与帧同尺寸）。只画 1 帧就是普通静态图标；2 帧以上"实际大小预览"会按设的帧间隔真的循环播放，插入到编辑框会生成 `icon.frames`。
    ...
    - **🎬 逐帧动画（icon.frames）**：不是靠变换（旋转/缩放）动，是像 Minecraft 皮肤那个会走路的角色一样，画好几张图交替显示——`frames` 给一组尺寸完全一致的图标网格，配一个 `frameDuration`（每帧播多久，默认 0.25 秒），就会按顺序循环播放，`musicReactive` 开着的话换帧节奏也跟着音乐变。帧数不设上限，想画几帧就几帧。目前只有主图标（装饰栏那个）支持，编辑页实时预览同步能看到效果；Mini 小方块/进度条图标/分享卡片/`layers` 暂时还是静态显示第一帧。
    - **🖱️ 点击切姿势（icon.actions）**：可选的一组"额外姿势"，点一下装饰栏图标（或者填了自动切换阈值，听够时长自动触发）就循环切到下一个，绕完一圈回到默认外观。每个姿势能自带一套帧/帧间隔（跟主图标一样支持逐帧动画）、一个独立的移动方式（可以覆盖主题顶层的呼吸发光/摇摆/旋转等，甚至换成 drift 三重影飘过 / fall 三重影飘落 / walk 在装饰栏来回走——姿势真的会搬到对应的轨道上，不只是换张图），还能配一个过渡时长（`transitionSeconds`）让切换变成慢慢淡化而不是瞬间跳变。写了 `icon.actions` 的主题，图标会带一圈轻微的呼吸描边提示"这里能点"，不用只靠鼠标移上去变手型光标才发现。图标画板里有专门的动作条（点格子画每个姿势的帧、勾选移动方式、拖拽调整循环顺序），"实际大小预览"本身也能点——点一下就模拟真实点击效果，画完当场就能验证，不用插入编辑框、套进真实播放器才知道效果对不对。
    - **🐾 联动 Mini 模式桌宠**：Mini 模式下点一下小方块，除了弹一句问候气泡，写了 `icon.actions` 的主题还会顺手切一次姿势——桌宠真的换个样子，不只是弹一下，展开回完整播放器时装饰栏图标也已经停在这次切到的姿势上，两边观感是连的。
...
├── MainWindow.PetMode.cs         Mini 模式小方块单击的"桌宠"反应：弹问候气泡 + 弹一下，写了 icon.actions 的客制化主题还会顺手切一次姿势
├── MiniAchievementRing.cs        Mini 模式成就环的纯几何/配色计算，带单元测试
├── MainWindow.Skins.cs           皮肤应用/调色板/客制化主题渲染/MC 皮肤 Steve 走路/客制化图标 icon.actions 切换与过渡淡化
