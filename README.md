...
- **启动自动记住上次的皮肤**：动画放完之后，只要本地已经选过一次（不是第一次打开），默认直接用上次的皮肤/尺寸/显示模式进播放器，不用每次都手动点"开始播放"。设置页最下面有个勾选框可以关掉这个行为，改回"每次启动都先看设置页"；改皮肤随时能从主播放器窗口右键菜单或者齿轮图标回去。
...
- **Mini 模式**：双击窗口缩成一个贴合当前皮肤主题的小方块（还能拖着走），再点一下展开，回到缩小前的原位。方块周围有两圈像素粒子跟着系统正在播放的真实音频跳动（WASAPI 回环采集，不是估算出来的）：内圈跟着整体响度一起呼吸，正常播放时就会动；外圈平时不显示，只有鼓点或者高音突然冲高时才会多冒出这一圈。设置页可以完全关掉这个效果，也能调"低/中/高"三档灵敏度；关掉之后 Mini 模式下不会启动任何音频采集。
...
- **歌词字号**：设置页里选小 / 中 / 大。
- **双语歌词**：左上角 🌐 开关，只有网易云那个引擎抓到翻译行时才会在原文下面多显示一行小字翻译，不是每首歌都有；本地缓存命中的歌词不带翻译（缓存只存原文，见代码注释）。
...
第一次打开会先弹出启动设置页，选好皮肤 / 尺寸 / 显示模式后点"开始播放"即可；之后再打开默认直接用上次的选择进播放器（设置页最下面能关掉这个行为）。主窗口右上角的齿轮图标（或右键菜单）可以随时回到设置页更换皮肤。

跑单元测试（LRC 解析、卡拉OK扫光估算、时长校验这几个不碰 UI/网络的纯逻辑）：
...
├── SettingsWindow.xaml(.cs)      启动设置页：皮肤/尺寸/显示模式/字号选择、缓存管理、手动检查更新
...
├── LyricsFetcher.cs              多引擎并发抓词（LRCLIB / 网易云 / QQ音乐 / 酷狗），网易云那个引擎顺带抓翻译行
...
└── *Tests.cs                     LrcParser / KaraokeTiming / LyricsFetcher.IsDurationPlausible 的单元测试（xUnit）
...
Mini 模式的像素粒子效果用 WASAPI 回环采集实时分析系统正在播放的声音，纯本地计算，不联网、不录制、不保存任何音频数据，用完（退出 Mini 模式/隐藏到托盘）就停止采集。设置页可以完全关掉这个功能，关掉后 Mini 模式下不会启动任何音频采集。
